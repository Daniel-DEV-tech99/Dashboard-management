...
// ** Actions Imports
import { updateTask } from 'src/store/apps/tasks'
...
// ** Schema Import
import { TaskSchema as schema } from '../add/validation'

interface SidebarEditTaskType {
  open: boolean
  toggle: () => void
  taskData: any | null
}
...
const EditTaskDrawer = (props: SidebarEditTaskType) => {
  // ** Props
  const { open, toggle, taskData } = props
  ...
  // Set default values based on taskData
  const defaultValues = {
    id: taskData?.id || '',
    title: taskData?.title || '',
    description: taskData?.description
      ? createEditorState(taskData.description)
  ...
  const onSubmit = (data: FormData) => {
    ...
    const taskUpdateData: TaskData = {
      ...data,
      description: descriptionHtml
    }

    try {
      dispatch(updateTask(taskUpdateData as any))
      toggle()
      reset()
